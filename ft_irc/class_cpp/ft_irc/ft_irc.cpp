...
void	ft_irc::start_polling( void )
{
	...
	while (42)
	{
		...
		{
			if (_fds[i].revents & POLLOUT && _fds[i].fd != _socketfd)
			{
				std::vector<user *>::iterator	user = std::find_if(_users.begin(), _users.end(), FindByFD(_fds[i].fd));

				if (user == _users.end() || (*user)->getMsgT().empty())
					continue ;
				(*user)->msgToUsser((*user)->getMsgT()[0]);
			}
